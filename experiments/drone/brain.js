...
class BrainModel {
  ...
  advance(input,seconds){
    ...
    for(let step=0;step<steps;step++){
      ...
      for(const name of ['LPLC2','LC4'])for(const side of ['left','right']){
        const drive=Math.max(0,Math.min(1,Number(input[side]?.[name.toLowerCase()])||0));
